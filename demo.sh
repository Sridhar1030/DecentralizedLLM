...
echo -e "\n=== 2. Stream Test (live token flow) ==="
echo "First 5 SSE events:"
curl -s -N -m 60 -X POST $BASE/v1/chat/completions/stream \
  -H "Content-Type: application/json" \
  -H "x-api-key: $KEY" \
  -d "{\"model\":\"$MODEL\",\"messages\":[{\"role\":\"user\",\"content\":\"Hi\"}],\"max_tokens\":5}" \
  2>/dev/null | head -15

echo -e "\n=== 3. Inference Request (model split across 3 devices) ==="
...
echo -e "\n=== 4. Invalid API Key (expect 403) ==="
curl -s -X POST $BASE/v1/chat/completions \
  -H "Content-Type: application/json" \
  -H "x-api-key: wrong-key" \
  -d '{"model":"x","messages":[]}' | jq .

echo -e "\n=== 5. Failover Demo: Stopping node1 (1/3 of model goes down) ==="
